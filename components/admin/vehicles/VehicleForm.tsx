    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="name" render={({ field }) => (
              <FormItem className="col-span-2">
                ...
                <FormControl><Input placeholder="e.g. Toyota Hiace 2023" {...field} style={{ background: 'var(--bg-overlay)', border: '1px solid var(--border-strong)', color: 'var(--text-primary)' }} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="capacity" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input type="number" min={1} max={100} {...field} style={{ background: 'var(--bg-overlay)', border: '1px solid var(--border-strong)', color: 'var(--text-primary)' }} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="province" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g. Phnom Penh" {...field} style={{ background: 'var(--bg-overlay)', border: '1px solid var(--border-strong)', color: 'var(--text-primary)' }} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            ...
            <FormField control={form.control} name="license_plate" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g. 2A-1234" {...field} value={field.value || ''} style={{ background: 'var(--bg-overlay)', border: '1px solid var(--border-strong)', color: 'var(--text-primary)' }} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
          </div>
        ...
        <div className="rounded-xl space-y-3" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', padding: '16px 20px' }}>
          <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Features</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {VEHICLE_FEATURES.map((feature) => (
              <label key={feature} className="flex items-center gap-2 rounded-lg p-2.5 cursor-pointer transition-colors"
                style={{ background: 'var(--bg-overlay)', border: `1px solid ${selectedFeatures.includes(feature) ? 'var(--brand-primary)' : 'var(--border-strong)'}` }}>
