...
export function RoomManagement({ hotelId }: RoomManagementProps) {
   ...
   *
   * `hotelsApi.deleteRoom` did not exist, and neither does DELETE on the backend.
   * Rooms are referenced by booking_items, so hard deletion would orphan booking
   * history; `isActive: false` takes the room out of inventory instead.
   */
  const deleteMutation = useMutation({
    mutationFn: (roomId: string) =>
      hotelsApi.updateRoom(hotelId, roomId, { isActive: false }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-hotel-rooms', hotelId] })
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        ...
        message={`Deactivate "${deleteTarget?.name}"? It will be removed from bookable inventory. Existing bookings are unaffected, and you can reactivate it later.`}
